...
* Declarations
  PARAMETER RP_RTF(R,P);
* Interpolate params
$IF %TST%==%1 OPTION %TST% < %2;
$ BATINCLUDE fillparm RCAP_BLK R 'P' ",'','','','',''" V 'RTP(R,V,P)' '>=0'
$ BATINCLUDE fillparm RCAP_OCAP R 'P,C' ",'0','0','0','0'" V 'RTP(R,V,P)' '>=0'
$ BATINCLUDE prepparm RCAP_BND R 'P,L' ",'','',''" V 'RTP(R,V,P)' 1
$IF DEFINED NCAP_OLIFE $BATINCLUDE fillparm NCAP_OLIFE R P ",'','','','',''" V 'RTP(R,V,P)' '>=0'
...
* Used OCAP flows only for RCAP
  RCAP_OCAP(RTP(R,V,P),C)$(PRC_RCAP(R,P)->0) = 0;
* Activate MIP solution if requested
$IFI %DSC%==YES $SETGLOBAL SOLMIP %RETIRE%
$IFI %RETIRE%==MIP $SETGLOBAL SOLMIP YES
$MACRO VNRET(V,T,P) RTP_CPTYR(R,VNTRT(V,T),P)
$EXIT
*-----------------------------------------------------------------------------
$ LABEL DECL
$ IF %MIP% INTEGER
...
* Force obj type 2a/2b
  VNTRT(VNT(V,T))$(NOT SAMEAS(V,T)) = YES;
  RVPRL(RTP(R,V,P))$PRC_RCAP(R,P)=MAX(0,SMAX(VNRET(V,T,P),YEARVAL(T))-YEARVAL(V));
  RVP(RTP) $= RVPRL(RTP);
  NCAP_ILED(RVP) = NCAP_ILED(RVP)+EPS;
* Sync variables
  OPTION TRACKP < RCAP_OCAP;
  RP_RTF(TRACKP)$(NOT RP_RTF(TRACKP)) = EPS;
  VN2T(VNTRT(LL,T(YEAR))) = ORD(YEAR)-ORD(LL);
* Set bounds for continuous retirements
  ...
  %VAR%_RCAP.LO(VNRET(V,T,P)%SOW%)$RVP(R,V,P) = RCAP_BND(R,T,P,'LO')$(NOT MAPVAL(RCAP_BLK(R,V,P)))+MIN(0,RTFORC(R,V,T,P)-RTFORC(R,V,TT,P)$Z);
  %VAR%_RCAP.UP(VNRET(V,T,P)%SOW%)$RCAP_BND(R,T,P,'UP') =
    MIN(SMIN(PASTMILE(V),NCAP_PASTI(R,V,P)),RCAP_BND(R,T,P,'UP')+MAX(0,RTFORC(R,V,T,P)-RTFORC(R,V,TT,P)$Z)));
  %VAR%_SCAP.LO(VNRET(V,T,P)%SOW%)$RVP(R,V,P) = MAX(RCAP_BND(R,T,P,'LO'),RTFORC(R,V,T,P));
  %VAR%_SCAP.UP(RTP_CPTYR(R,PASTMILE(V),T,P)%SOW%)$RVP(R,V,P) = MAX(RTFORC(R,V,T,P),NCAP_PASTI(R,V,P));
  LOOP(RVP(R,V,P),Z=1;LOOP(VNRET(V,T,P)$Z,Z=0; %VAR%_SCAP.UP(R,V,T,P%SOW%)=MIN(%VAR%_RCAP.UP(R,V,T,P%SOW%),%VAR%_SCAP.UP(R,V,T,P%SOW%))));
...
* Force refits = retirements on request
  ...
  OPTION CLEAR=TRACKP,CLEAR=PASTSUM,CLEAR=RVP;
$EXIT
*-----------------------------------------------------------------------------
$LABEL EQOBJ
$SETGLOBAL RCAPSUB -SUM(VNTRT(V,T),%VART%_SCAP(R,V,T,P%SWS%))$PRC_RCAP(R,P)
$SETGLOBAL RCAPSBM -SUM(VNTRT(MODLYEAR,T),%VART%_SCAP(R,MODLYEAR,T,P%SWS%))$PRC_RCAP(R,P)
*-------------------

* Allow retirements in integer multiples of a user-defined block-size or the full residual capacity
  %EQ%_DSCRET(RTP_CPTYR(%R_V_T%,P)%SWT%)$(VNTRT(V,T)$RCAP_BLK(R,V,P))..
...
* Cumulative retirements
  %EQ%_CUMRET(R,VNTRT(V,K(T-(1-1$RP_RTF(R,P)))),P%SWT%)$(RTP_CPTYR(R,V,T,P)$PRC_RCAP(R,P))..

   SUM(RTP_CPTYR(R,V,K,P),%VAR%_SCAP(R,V,T,P%SOW%) - %VAR%_RCAP(R,V,T,P%SOW%)) -
   SUM(RTP_CPTYR(R,V,MODLYEAR(Y(T-1)),P)$VNTRT(V,Y),%VARM%_SCAP(R,V,Y,P%SWS%))
   =E= 0;
...
* Maximum salvage capacity
  %EQ%L_SCAP(RTP(R,V(LL),P),IPS%SOW%)$(((OBJ_SUMS(RTP)+(NOT PRC_VINT(R,P))$T(V))$RVPRL(RTP)$L(IPS) OR NCAP_OLIFE(RTP)$IO(IPS))$PRC_RCAP(R,P))..
   ...
   SUM(VNTRT(V,T(LL+RVPRL(RTP))),%VART%_SCAP(R,V,T,P%SWS%))$LIM(IPS)

   =L=  %VARV%_NCAP(R,V,P%SWS%)$T(V) + NCAP_PASTI(R,V,P) - %VAR%_SCAP(R,V,'0',P%SOW%)$RVPRL(RTP);
...
* Retrofits and life-extensions
  %EQ%L_REFIT(RTP_TT(R,TT,T,PRC),LNX(L)%SWT%)$((BD(L)+VNT(T,TT)$(RP_RTF(R,PRC)=3))$RT_PP(R,T))..
   ...
   =E=
   SUM(VNRET(V,TT,PRC),COEF_CPT(R,V,T,PRC) * (%VARTT%_RCAP(R,V,TT,PRC%SWS%)-SUM(MODLYEAR(K(TT-1))$VNT(V,K),MIN(INF$MOD(RP_RTF(R,PRC),2),RTFORC(R,V,TT,PRC)-RTFORC(R,V,K,PRC)))))+
...
*  Define upper bound for integer multiples
  ...
  RCAP_OCAP(RTP,C)$(NOT RVPRL(RTP)) = 0;
  OPTION CLEAR=COEF_CAP;
$EXIT
*-----------------------------------------------------------------------------
$LABEL OBJFIX
* Credit retired capacity for the avoided fixed costs
   SUM((OBJ_SUMIV(K_EOH,R,V,P,JOT,LIFE),VNTRT(V,T))$RVPRL(R,V,P),
