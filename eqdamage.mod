...
*-----------------------------------------------------------------------------
* damages
$ IF NOT DEFINED DAM_ELAST PARAMETER DAM_ELAST(REG,COM,LIM)  //;
...
* Ensure that elasticities are greater than or equal to 1:
  DAM_ELAST(TRACKC(R,C),'LO') = ABS(DAM_ELAST(R,C,'UP'))$(NOT DAM_ELAST(R,C,'LO'))+ABS(DAM_ELAST(R,C,'LO'));
  DAM_ELAST(TRACKC(R,C),'UP') = ABS(DAM_ELAST(R,C,'LO'))$(NOT DAM_ELAST(R,C,'UP'))+ABS(DAM_ELAST(R,C,'UP'));

* Adjust VOCs
  DAM_TVOC(RXX(R,T,C),BD)$(NOT DAM_TVOC(R,T,C,BD)) $= (DAM_STEP(R,C,BD)+.5)*DAM_VOC(R,C,'N')*DAM_TQTY(R,T,C);
  DAM_TVOC(RXX(R,T,C),'LO')$DAM_TVOC(R,T,C,'LO') = MIN(DAM_TQTY(R,T,C),DAM_TVOC(R,T,C,'LO'));
  DAM_TVOC(RXX(R,T,C),'LO')$(NOT DAM_TVOC(R,T,C,'LO')) = DAM_TQTY(R,T,C);
  ...
  DAM_SIZE(RXX(R,T,C),'LO')$DAM_STEP(R,C,'LO') = MAX(0,(DAM_TVOC(R,T,C,'LO')-0.5*DAM_SIZE(R,T,C,'FX')))/DAM_STEP(R,C,'LO');
  DAM_SIZE(RXX(R,T,C),'UP')$DAM_STEP(R,C,'UP') = MAX(0,(DAM_TVOC(R,T,C,'UP')-0.5*DAM_SIZE(R,T,C,'FX')))/DAM_STEP(R,C,'UP');
