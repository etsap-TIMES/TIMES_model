...
*=============================================================================*
OPTION CLEAR=UNCD7;
$SETLOCAL TAKE '' SETLOCAL REGY 'R,%7' SETLOCAL TS (NOT %3(R,%2,S))$
...
$IF '%11'=='N' %1(R,LL--ORD(LL),%2%10,S+STOA(S)%4)$(RS_BELOW(R,'ANNUAL',S)$%1(R,LL,%2%10,S%4)) = 1;
*-----------------------------------------------------------------------------
LOOP((%REGY%,%2%10,S%4)$(%TS%%1(%REGY%,%2%10,S%4)),
* Mark to be levelized if inheritance is allowed or S is not ANNUAL:
  UNCD7(%REGY%,%2%10%4%5)$(%8%TAKE%) = YES);
*-----------------------------------------------------------------------------
    ...
    %1(%REGY%,%2%10,TS%4) $=
     SUM(TS_MAP(R,TS,S)$FINEST(R,S), %AWGT% * (TS_ARRAY(S) + 
            SUM(RS_BELOW(R,%6,S)$(TS_ARRAY(%6) AND NOT SUM(SL$RS_BELOW(R,%6,SL),TS_MAP(R,SL,S)*TS_ARRAY(SL))),
                       TS_ARRAY(%6)%IWGT%)));)));
*-----------------------------------------------------------------------------
$IF '%11'=='N' %1(R,LL,%2%10,S%4)$((NOT %3(R,%2,S))$%1(R,'0',%2%10,'ANNUAL'%4)) = 0;
